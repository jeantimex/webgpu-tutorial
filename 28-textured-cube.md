...
Every vertex in our buffer now needs a pair of UV coordinates. Since a single corner of the cube needs different UVs depending on which face it belongs to (e.g., $(0,0)$ for the top-left of the Front face, but $(1,0)$ for the top-right of the Side face), we must use **24 vertices** (4 per face), just like we did for [Cube Face Colors](27-cube-face-colors.html).
