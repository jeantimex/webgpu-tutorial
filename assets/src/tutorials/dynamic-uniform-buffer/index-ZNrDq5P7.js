import"../../../modulepreload-polyfill-B5Qt9EMX.js";/* empty css                      */import{i as G}from"../../../webgpu-util-BApOR-AX.js";async function S(){const a=document.querySelector("#webgpu-canvas"),{device:e,context:P,canvasFormat:u}=await G(a),m=new Float32Array([0,.5,0,-.5,-.5,0,.5,-.5,0]),l=e.createBuffer({label:"Vertex Buffer",size:m.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(l,0,m);const d=e.limits.minUniformBufferOffsetAlignment,s=5,p=16,f=Math.ceil(p/d)*d,g=s*f,y=e.createBuffer({label:"Dynamic Uniform Buffer",size:g,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),i=new Float32Array(g/4);for(let r=0;r<s;r++){const v=Math.random(),c=Math.random(),n=Math.random(),t=r*f/4;i[t+0]=v,i[t+1]=c,i[t+2]=n,i[t+3]=1}e.queue.writeBuffer(y,0,i);const o=e.createShaderModule({label:"Dynamic Uniform Shader",code:`
      struct Uniforms {
        color : vec4f,
      };
