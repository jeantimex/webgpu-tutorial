...
function createConeTypedArrays(
  radius: number,
  height: number,
  ...
  if (!openEnded && radiusBottom > 0) {
    ...
    for (let x = 0; x <= radialSegments; x++) {
      ...
      positions.push(radius * sinTheta, -halfHeight, radius * cosTheta);
      normals.push(0, -1, 0);
    }
...
async function init() {
  ...
  let model = createVertexAndIndexBuffer(
    device,
    createConeTypedArrays(
      coneSettings.radius,
      coneSettings.height,
      coneSettings.radialSegments,
      coneSettings.heightSegments,
      coneSettings.openEnded,
      coneSettings.thetaStart * Math.PI,
      coneSettings.thetaLength * Math.PI
    )
  );
  ...
  const renderPassDescriptor: GPURenderPassDescriptor = {
    label: "cone wireframe render pass",
    colorAttachments: [
      {
        view: undefined,
  ...
  gui.add(coneSettings, "radius", 0, 40).onChange(rebuildCone);
  gui.add(coneSettings, "height", 5, 80).onChange(rebuildCone);
  gui.add(coneSettings, "radialSegments", 3, 64, 1).onChange(rebuildCone);
  gui.add(coneSettings, "heightSegments", 1, 32, 1).onChange(rebuildCone);
  gui.add(coneSettings, "openEnded").onChange(rebuildCone);
  gui
    .add(coneSettings, "thetaStart", 0, 2)
    .name("thetaStart (x PI)")
    .onChange(rebuildCone);
  gui
    .add(coneSettings, "thetaLength", 0, 2)
