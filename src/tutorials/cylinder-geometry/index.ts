...
import wireframeShaderWGSL from "./wireframe.wgsl?raw";

const solidColorLitWGSL = `${litVertexWGSL}\n${litFragmentWGSL}`;

const wireframeWGSL = wireframeShaderWGSL;
...
function createCylinderTypedArrays(
  radiusTop: number,
  radiusBottom: number,
  height: number,
  ...
  function generateCap(top: boolean) {
    ...
    for (let x = 0; x < radialSegments; x++) {
      const curr = ringStart + x;
      const next = ringStart + x + 1;
      if (top) {
...
async function init() {
  ...
  let model = createVertexAndIndexBuffer(
    device,
    createCylinderTypedArrays(
      cylinderSettings.radiusTop,
      cylinderSettings.radiusBottom,
      cylinderSettings.height,
      cylinderSettings.radialSegments,
      cylinderSettings.heightSegments,
      cylinderSettings.openEnded,
      cylinderSettings.thetaStart * Math.PI,
      cylinderSettings.thetaLength * Math.PI
    )
  );
  ...
  const renderPassDescriptor: GPURenderPassDescriptor = {
    label: "cylinder wireframe render pass",
    colorAttachments: [
      {
        view: undefined,
  ...
  gui.add(cylinderSettings, "radiusTop", 0, 40).onChange(rebuildCylinder);
  gui.add(cylinderSettings, "radiusBottom", 0, 40).onChange(rebuildCylinder);
  gui.add(cylinderSettings, "height", 5, 80).onChange(rebuildCylinder);
  gui.add(cylinderSettings, "radialSegments", 3, 64, 1).onChange(rebuildCylinder);
  gui.add(cylinderSettings, "heightSegments", 1, 32, 1).onChange(rebuildCylinder);
  gui.add(cylinderSettings, "openEnded").onChange(rebuildCylinder);
  gui
    .add(cylinderSettings, "thetaStart", 0, 2)
    .name("thetaStart (x PI)")
    .onChange(rebuildCylinder);
  gui
    .add(cylinderSettings, "thetaLength", 0, 2)
  ...
  function render(ts: number) {
    ...
    const view = mat4.lookAt([0, 0, 90], [0, 0, 0], [0, 1, 0]);
