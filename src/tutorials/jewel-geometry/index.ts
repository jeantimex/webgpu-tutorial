...
function createJewelTypedArrays(
  radius: number,
...
async function init() {
  ...
  let model = createVertexAndIndexBuffer(
    device,
    createJewelTypedArrays(
      jewelSettings.radius,
      jewelSettings.widthSegments,
      jewelSettings.heightSegments
    )
  );
  ...
  const renderPassDescriptor: GPURenderPassDescriptor = {
    label: "jewel wireframe render pass",
    colorAttachments: [
      {
        view: undefined,
  ...
  gui.add(jewelSettings, "radius", 5, 40).onChange(rebuildJewel);
  gui.add(jewelSettings, "widthSegments", 3, 24, 1).onChange(rebuildJewel);
  gui.add(jewelSettings, "heightSegments", 2, 24, 1).onChange(rebuildJewel);
